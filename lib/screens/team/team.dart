                    ...
                    ? Expanded(
                        ...
                        child: PageView.builder(
                          ...
                          onPageChanged: (index) async {
                            ...
                            if (index == 0) {
                              title = CORE_TEAM;
                              await getTeamData(data: CORE_TEAM);
                            } else if (index == 1) {
                              title = WEB_TEAM;
                              await getTeamData(data: WEB_TEAM);
                            } else if (index == 2) {
                              title = ML_TEAM;
                              await getTeamData(data: ML_TEAM);
                            } else if (index == 3) {
                              title = ANDROID_TEAM;
                              await getTeamData(data: ANDROID_TEAM);
                            } else if (index == 4) {
                              title = DESIGN_TEAM;
                              await getTeamData(data: DESIGN_TEAM);
                            } else if (index == 5) {
                              title = CONTENT_TEAM;
                              await getTeamData(data: CONTENT_TEAM);
                            } else if (index == 6) {
                              title = MANAGEMENT_TEAM;
                              await getTeamData(data: MANAGEMENT_TEAM);
                            } else {
                              title = CORE_TEAM;
                              await getTeamData(data: CORE_TEAM);
                            }
                              ...
                              itemBuilder: (context, index) {
                                return Container(
                                  margin: EdgeInsets.only(
                                    ...
                                    right: 36,
                                    top: 10,
                                    bottom: teamMembers.length - 1 == index
                                        ? 100
                                        : 10,
                                  ),
                                  height: 300,
                                  decoration: BoxDecoration(
                                    ...
                                    image: DecorationImage(
                                      ...
                                      onError: (exception, stackTrace) {
                                        return print("unable to load image");
                                      },
                                    ),
                                  ),
                                  child: Column(
